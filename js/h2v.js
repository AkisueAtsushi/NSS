...
var h2vconvert = {
	...
	startConvert : function( params ){
		...
		if( hc.envs.isOld ){
			for( id in params ){
				if( tyz.getById( id ) ){
					hc.addBar( id , '最新のブラウザでは縦書きで閲覧できます' );
				}
			}
			return false;
		}
	...
	convert : function( id ){
		...
		if( tgt.converted == '' ){		//未変換なら変換。
			...
			if( !hc.envs.isIE8 ){		// pre要素の中身以外の cr / lf / tab 削除
			...
			var lines = hc.separateLines( id , tgt.html , tgt.fsPX , lineH );
	...
	separateLines : function( id , basenode , baseFont , maxHeight ){
		...
		while( df = hc.seekNodes( id , basenode , baseFont , mH ) ){
			if( hc.isBlock( df ) || hc.isList( df ) || hc.isThru( df )){
	...
	setTextAlign : function( line , mH , tgt ,baseFont, jfc ){
		...
		var d ,s=0,t=0,c,l,e,eh=0,sh=0,ec;
		...
		if( tgt.tAlign == 3 ){//justify処理
			if( (d=mH-line.offsetHeight) > 0 ){
				...
				if( (eh = hc.getExtraHeight(d,t))>0 ){line.style.lineHeight = baseFont+eh+'px';}
				d-=(t*eh);
	...
	seekNodes : function( id , node , baseFont , maxHeight){
		...
		for( i = 0 ; i < node.childNodes.length ; i++ ){
			...
			switch(nodename){
				case '#text' :
					...
					if( tyz.ceil( extraheight , 2 ) <= tyz.floor( nodeheight , 2 ) ){
						var el = Math.floor( extraheight / baseFont) ;
						if( el == 0 ){			// 入らない。
				...
				case 3 :/*h2v_cho, h2v_cho_r, h2v_komoji, h2v_rotate*/
					tgt.cH +=baseFont ;
				...
				case 6 : /*h2v_alpha*/
					...
					tgt.cH+=nodeheight ;
					if( !cn['done_h2v'] )tyz.setStyles( cn, { 'width':nodeheight+'px','marginBottom' : Math.ceil(nodeheight - baseFont)+'px' } );
				...
				case 7:
				case 'RB': case 'RT': case 'RTC': case 'RBC':
					chbk = tgt.cH ;
					hc.seekNodes( id , cn , baseFont , 1000 ) ;
					tgt.cH = chbk;
				...
				case'CODE':case'VAR':case'SAMP':case'KBD':
					...
					var newProp = hc.getNewProp( cn , id ,true),
						ds = newProp.ds,
					...
					var mh = maxHeight - tyz.PX( ds.marginLeft)- tyz.PX( ds.paddingLeft ) -tyz.PX( ds.borderLeftWidth ) ;
					if( mh <= 0 )mh = fs;

					hc.setLineMargin( id , fs , ds );

					var df = hc.seekNodes( id , cn , fs , mh ) ;
					if( df ) {
					...
					tgt.cH+=newProp.xm*1;
					if(!cn.done_h2v)tyz.setStyles( cn , newProp.s );
					cn['done_h2v']=true;
					break;
				...
				case'ADDRESS':case'ARTICLE':case'ASIDE':case'FOOTER':case'HEADER':case'HGROUP':case'NAV':case'DETAILS':case'SECTION':case'SUMMARY':
					...
					var newProp = hc.getNewProp( cn , id ),
						ds = newProp.ds,
						fs = tyz.PX( ds.fontSize );
					...
					var mh = maxHeight - newProp.xm ;
					if( mh <= 0 ) mh = fs;

					hc.setLineMargin( id , fs );

					var lines = hc.separateLines( id , cn , fs , mh );
				...
				case 'RUBY' :
					...
					if( rbc.length == 0 ){
						...
						if( rtc.length == 0 )break;
						rb = tyz.getByTag( rbc[0] , 'RB' ),
						rt = tyz.getByTag( rtc[0] , 'RT' );
						if( rb.length != rt.length )break;
						for( rl = 0 ; rl < rb.length ; rl ++ ){
							rbH = rb[rl].offsetHeight;
							rtH = rt[rl].offsetHeight;
							H = rbH-rtH > 0 ? rbH : rtH ;
							nH += nH ;
							if( rbH-H < 0 )tyz.setStyles( rb[rl] , { 'marginTop' : ( H - rbH ) / 2 + 'px' , 'height' : rbH*1 + ( H - rbH ) / 2 + 'px' } );
							if( rtH-H < 0 )tyz.setStyles( rt[rl] , { 'marginTop' : ( H - rtH ) / 2 + 'px' , 'height' : rtH*1 + ( H - rtH ) / 2 + 'px' } );
						}
					}
				...
				default :
					var df = hc.seekNodes( id , cn , baseFont , maxHeight ) ;
					if( df ) return df;
					break;
			}//nodename
			...
			if(cn.nextSibling ){
				...
				if( hc.isBlock( ns ) || hc.isThru( ns )){return tgt.okuri||cn;}
			}
		}
			...
			for( i=0;i<cn.length;i++){
				...
				if( h2vconvert.isThru(cn[i])||
							h2vconvert.span_match(cn[i],'h2v_e')||
								h2vconvert.isInline(cn[i])||
									(h2vconvert.isBlock(cn[i])&&!cn[i].hasChildNodes() ) ){//分割できない要素の時
					cw+=W;
	...
	getNewProp : function( cn , id , flag){
		var ds = h2vconvert.getDispS( cn , ['fontSize',
			...
			newMargin = tyz.PX( ds.marginLeft)+'px '+tyz.PX(ds.marginTop)+'px '+tyz.PX(ds.marginRight)+'px '+tyz.PX(ds.marginBottom)+'px',
			newPadding = tyz.PX(ds.paddingLeft)+'px '+tyz.PX(ds.paddingTop)+'px '+tyz.PX(ds.paddingRight)+'px '+tyz.PX(ds.paddingBottom)+'px' ,
			newStyles = { 'margin' : newMargin , 'padding' : newPadding ,'fontSize':tyz.PX(ds.fontSize)+'px'} ,
	...
	ruby_enabler : function( id ){	// Modified at July 2013 ver 3.2.1
		...
		if( /webkit/.test(ua)|| envs.isIE ){			/* webkit & IE : html5の記述に合わせる */
			var rbc,rtc,rb,rt;
			for( i = ruby.length-1 ; i >= 0 ; i-- ){
				...
				if(rb.length != 0 ){
					...
					for(j=0;j<rt.length;j++ ){
						newRUBY = tyz.newEle('RUBY');
						newRUBY.appendChild( rt[j].previousSibling.cloneNode(true) );
						newRUBY.appendChild( rt[j].cloneNode(true) );
						pRUBY.appendChild(newRUBY.cloneNode(true));
					}
				}
			...
			for( var i = 0 ; i < cn.childNodes.length ; i++ ){
				...
				switch( ccn.nodeName ){
					case"#text":
						...
						newRB.innerHTML = '<span class="psdo_r_inner">'+newRB.innerHTML + '</span>';
...
var h2v_ie = {
	...
	getFontSize : function( node ){
		var tt = tyz.newEle( 'DIV' , {} , { 'width' : '10em' } ),
			fs;
		try{
			node.appendChild( tt );
	...
	setTextAlign : function( line , mH , tgt ,baseFont, jfc ){
		...
		if( tgt.tAlign==3||tgt.tAlign==0)return line;
		line_p = tyz.newEle( 'DIV' , {} , { 'fontSize' : baseFont + 'px' } );
		line_p.appendChild( line );
		hc.tmpdiv.appendChild( line_p );
		mgn = ( mH - line.offsetHeight ) / tgt.tAlign;
		line.style.marginTop = tyz.PX(line.style.marginTop) + mgn + 'px' ;
		hc.tmpdiv.removeChild( line_p );
		return line ;	
	}
};
...
(function(){
		...
		if( /msie/.test(ua) ) envs.isIE = true;
		...
		if( envs.isIE8 ){/*replace functions*/
			...
			hc.setTextAlign = h2v_ie.setTextAlign; //added at Jul 2013 ver 3.3.0. 
		...
		h2vconvert.ptn = RegExp( '([' + hc.komojis + '])|([' + hc.rotate_mirrors + '])|([' + hc.kutos + '])|([\\s　])|([!-~‘”“’]+\\s?)|([' + kakkos_t + '])|([' + kakkos_b + '])|(['+ hc.kakkos_tr + '])|([' + hc.kakkos_br + '])|(['+hc.rotates+'])|([！？])' , 'g' );
